...
describe('/1/users', () => {
  ...
  describe('POST /me/mfa/generate-codes', () => {
    it('should generate new codes', async () => {
      const user = await createUser({});
      const response = await request('POST', `/1/users/me/mfa/generate-codes`, {}, { user });
      expect(response.status).toBe(200);
      expect(response.body.data[0]).toBeDefined();
    });
  });

  describe('POST /me/mfa/verify', () => {
