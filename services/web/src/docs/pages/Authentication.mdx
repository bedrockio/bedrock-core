...
<Route route="POST /1/auth/register" />
...
<Route route="POST /1/auth/login/send-sms" />

### Verify sms

This verifies the user's one time code and returns a `data.token` on success

<Route route="POST /1/auth/login/verify-sms" />
...
<Route route="POST /1/auth/request-password" />
...
<Route route="POST /1/auth/set-password" />
